...
const TimelineEvent = ({ event, index }: any) => {
  ...
  return (
    ...
    >
      ...
      <motion.div
        className="z-20 flex items-center order-1 bg-purple-100 shadow-xl w-12 h-12 rounded-full"
        whileHover={{ scale: 1.2, rotate: 360 }}
        transition={{ type: "spring", stiffness: 260, damping: 20 }}
      >
        <event.icon className="w-6 h-6 text-black mx-auto" />
      </motion.div>
      <motion.div
        className="order-1 bg-purple-text-purple-100 rounded-lg shadow-xl w-5/12 px-6 py-4 backdrop-blur-sm bg-opacity-80"
        whileHover={{
          scale: 1.05,
          boxShadow: "0 0 20px rgba(51, 109, 234, 0.3)",
        }}
        transition={{ type: "spring", stiffness: 300, damping: 20 }}
      >
        <h3 className="mb-3 font-bold text-purple-100 text-xl">
          {event.title}
        </h3>
        <p className="text-sm leading-snug tracking-wide text-purple-100 text-opacity-100">
          {event.description}
        </p>
        <p className="mt-2 text-xs text-purple-100 font-semibold">
          {event.date}
        </p>
      </motion.div>
    </motion.div>
    ...
    <Section id="services">
      <div ref={ref} className="container mx-auto px-4 py-16  min-h-screen">
        ...
        <div className="relative wrap overflow-hidden p-10 h-full">
          <motion.div
            className="border-2-2 absolute border-purple-text-purple-100 h-full border"
            style={{
              left: "50%",
              height: lineHeight,
